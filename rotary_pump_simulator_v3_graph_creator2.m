...
[time_vector, graph_values_matrix] = simulate(600, 0.01, 1 ,10, 0.02 * (0.0254), 100, 5.22, 104.2 * (2*pi/60), 6000);
plot(time_vector(1,:), graph_values_matrix(1,:));

[time_vector, graph_values_matrix] = simulate(600, 0.01, 1 ,10, 0.08 * (0.0254), 100, 5.22, 104.2 * (2*pi/60), 6000);
plot(time_vector(1,:), graph_values_matrix(1,:));

[time_vector, graph_values_matrix] = simulate(600, 0.01, 1 ,10, 0.15 * (0.0254), 100, 5.22, 104.2 * (2*pi/60), 6000);
plot(time_vector(1,:), graph_values_matrix(1,:));

yline(C.pump_minimum_angular_velocity*60/(2*pi));
legend('r_{spool} = 0.02 in',...
    'r_{spool} = 0.08 in (optimal)',...
    'r_{spool} = 0.15 in');
...
function [steady_state_velocity] = get_efficiency(simulation_max_time, simulation_time_interval, ...
    ...
    steady_state_velocity = graph_values_matrix(simulation_max_time);
end
